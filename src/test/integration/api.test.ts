...
describe('API Integration Tests', () => {
  describe('Checkout API', () => {
    ...
    it('should accept valid packageId 10', async () => {
      ...
      const response = await fetch('/api/checkout', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ packageId: '10', userId: 'test-user', userEmail: 'test@example.com' }),
      });
    ...
    it('should accept valid packageId 100', async () => {
      ...
      const response = await fetch('/api/checkout', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ packageId: '100', userId: 'test-user', userEmail: 'test@example.com' }),
      });
    ...
    it('should accept valid packageId 500', async () => {
      ...
      const response = await fetch('/api/checkout', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ packageId: '500', userId: 'test-user', userEmail: 'test@example.com' }),
      });
  ...
  describe('Verify API', () => {
    ...
    it('should handle verification with different strategies', async () => {
      const strategies = ['graph_exact', 'vector_semantic', 'hybrid', 'cascading'];
    ...
    it('should verify multiple texts', async () => {
      server.use(
        http.post('*/api/v1/verify/batch', () => {
          return HttpResponse.json({
            results: [
              { id: '1', trust_score: { score: 0.9 }, claims: [], processing_time_ms: 50, cached: false },
              { id: '2', trust_score: { score: 0.85 }, claims: [], processing_time_ms: 60, cached: false },
              { id: '3', trust_score: { score: 0.95 }, claims: [], processing_time_ms: 40, cached: false },
            ],
      ...
      const response = await fetch('http://localhost:8000/api/v1/verify/batch', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
