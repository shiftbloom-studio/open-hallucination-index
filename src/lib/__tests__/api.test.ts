...
import { mockHealthResponse, mockVerifyResponse, mockBatchVerifyResponse } from '@/test/mocks/handlers';
...
describe('ApiClient', () => {
  ...
  describe('verifyText', () => {
    ...
    it('should throw error with status and message on failure', async () => {
      ...
      await expect(
        client.verifyText({ text: '' })
      ).rejects.toThrow('Verification failed: 400 Invalid request format');
    });

    it('should handle different strategies', async () => {
      const strategies = ['graph_exact', 'vector_semantic', 'hybrid', 'cascading'] as const;
  ...
  describe('verifyBatch', () => {
    ...
    it('should send correct batch request body', async () => {
      ...
      server.use(
        http.post('http://test-api.com/api/v1/verify/batch', async ({ request }) => {
          capturedBody = await request.json() as Record<string, unknown>;
          return HttpResponse.json(mockBatchVerifyResponse);
        })
      );
    ...
    it('should throw error on batch verification failure', async () => {
      ...
      await expect(
        client.verifyBatch({ texts: ['Test'] })
      ).rejects.toThrow('Batch verification failed');
    });
  });
});
