...
class VerifyTextUseCase:
    ...
    async def execute(
        ...
        *,
        ...
        # Step 4: Build ClaimVerification objects with score contributions
        claim_verifications: list[ClaimVerification] = []
        for claim, (status, trace) in zip(claims, verification_results, strict=True):
            # Create preliminary verification to compute contribution
            preliminary = ClaimVerification(
                claim=claim,
                status=status,
                trace=trace,
                score_contribution=0.0,  # Placeholder
            )
            contribution = await self._scorer.compute_claim_contribution(preliminary)
